#!/usr/bin/env python3
"""
统计各策略在历史数据上的原始命中数（无Top10截断，全市场）
用于对比当前实时筛选的命中数是否合理
"""
import sys, os, json
...
def check_signal(df_hist, strat):
    try:
        close = df_hist['close']
        high  = df_hist['high']
        low   = df_hist['low']
...
def run_stats():
    ...
    start, end = datetime(2026, 3, 15), datetime(2026, 4, 24)
    ...
    print(f'信号日: {n_dates} 个 ({dates_str})')
    print(f'策略: {n_strats} 个')
    print()
    sys.stdout.flush()

    # 统计
    weekly_counts = {s: [] for s in strategies}
    ...
    for tdate in dates:
        for strat in strategies:
            ...
            for code, df in stock_dfs.items():
                ...
                if check_signal(hist, strat):
                    week_hits += 1
            weekly_counts[strat].append(week_hits)
            print(f'  {tdate} {strat}: {week_hits}')
            sys.stdout.flush()

    # 汇总
    print()
    print('=' * 95)
    print(f"{'策略':<18} {'周数':>6} {'总命中':>10} {'周均':>8} {'月均':>8} {'周最低':>8} {'周最高':>8}")
    print('-' * 95)

    for strat in strategies:
        counts = weekly_counts[strat]
        label = STRATEGY_REGISTRY[strat]['name']
        total_hits = sum(counts)
        avg_week = total_hits / len(counts) if counts else 0
        avg_month = avg_week * 4.3
        min_week = min(counts) if counts else 0
        max_week = max(counts) if counts else 0
        print(f'{label:<18} {len(counts):>6} {total_hits:>10} {avg_week:>8.1f} {avg_month:>8.1f} {min_week:>8} {max_week:>8}')

    print('=' * 95)
    print()
    print('注：月均 = 周均 x 4.3（每月约4.3周）')
    sys.stdout.flush()
