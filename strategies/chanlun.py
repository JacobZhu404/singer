...
缠论选股策略
核心概念：
  1. 分型 (Fractal): 顶分型（连续三根K线，中间最高）/ 底分型（连续三根，中间最低）
  2. 笔 (Stroke): 方向明确的笔段，由相邻的顶/底分型构成，至少5根K线
  3. 中枢 (Zhongshu/Pivot): 三段重叠形成的震荡区间
  4. 背驰: 价格创新低但MACD/量能不创新低 → 潜在转折
  5. 三类买点:
     - 一买: 下跌背驰后的第一低点
     - 二买: 回调不破一买点（中枢下沿支撑）
     - 三买: 向上离开中枢后的回调不进入中枢
...
class ChanlunStrategy(BaseStrategy):
    ...
    """
    name = "chanlun"
    description = "缠论信号：底分型 + MACD背驰 + 中枢支撑 + 三类买点"
    base_win_rate = 0.58
